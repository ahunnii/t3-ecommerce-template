  ...
  ({ hasButton = true, ...props }, ref) => {
    return (
      <ContentSwitchWrapper {...props} ref={ref}>
        {props?.subtitle && (
          <p className="absolute bottom-32 left-5 z-[45]  text-lg font-semibold text-white lg:text-xl ">
            {props?.subtitle}
          </p>
        )}

        <p className="absolute bottom-20 left-5  z-[45]  text-2xl font-semibold text-white lg:text-3xl">
          {props?.title}
        </p>
        {hasButton && (
          <Button
            className="absolute bottom-5 left-5 z-[45]"
            variant={"outline"}
          >
            Shop {props.title}
          </Button>
